...
Output: User model extension, APScheduler integration, profile settings for notifications.
</objective>
...
@.planning/phases/03-free-horoscopes/03-01-SUMMARY.md
@src/db/models/user.py
@src/main.py
@src/config.py
@src/bot/handlers/menu.py
...
python -c "from src.db.models.user import User; print([c.name for c in User.__table__.columns if 'notif' in c.name or 'time' in c.name])"
ls migrations/versions/*notification*
```
Должно показать: apscheduler установлен, поля есть в модели, миграция создана.
  </verify>
  <done>
...
async def settings_notifications(message: Message, session: AsyncSession) -> None:
    ...
    await message.answer(text, reply_markup=build_notifications_toggle_keyboard(user.notifications_enabled))
...
5. Обновить `src/bot/handlers/menu.py` — добавить кнопку "Настройки уведомлений" в Профиль:
В `menu_profile` добавить кнопку:
```python
from aiogram.utils.keyboard import ReplyKeyboardBuilder
# ...
# После вывода профиля, добавить кнопку настроек
builder = ReplyKeyboardBuilder()
builder.button(text="Настройки уведомлений")
builder.button(text="Назад")
builder.adjust(1)
# Или использовать InlineKeyboardBuilder для inline кнопки
```

Проще: добавить в текст профиля информацию и отдельную кнопку.
Лучше: использовать inline keyboard под сообщением профиля.

Реализация: добавить inline keyboard в menu_profile с кнопкой "Настройки уведомлений".
...
```bash
python -c "
from src.bot.keyboards.profile import build_timezone_keyboard, build_notification_time_keyboard
