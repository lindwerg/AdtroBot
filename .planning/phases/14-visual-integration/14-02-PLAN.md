...
2. Модифицировать `cmd_start(message, session)` -> `cmd_start(message, session, bot: Bot)`:
   - Добавить параметр `bot: Bot` (aiogram 3.x inject'ит автоматически)
...
2. Модифицировать `show_zodiac_horoscope(callback, callback_data, session)` -> добавить `bot: Bot`:
   - ПЕРЕД edit_text с гороскопом — отправить изображение
   - Использовать callback.message.answer_photo (не edit) — edit_text не может добавить фото
   - Затем edit_text с гороскопом

   Проблема: edit_text на сообщении с фото не добавит фото.
   Решение: Отправить изображение ОТДЕЛЬНЫМ сообщением через send_random_cosmic, затем edit_text для текста.
   ...
   # Затем существующий edit_text
   ```

3. Модифицировать `show_horoscope_message(message, sign_name, user_sign, session)` -> добавить `bot: Bot`:
   - Отправить изображение перед гороскопом
   - Это функция вызывается из start.py при первом гороскопе

4. Важно: НЕ отправлять изображение если это edit существующего сообщения.
   Добавить проверку — если вызывается через callback (edit), изображение уже отправлено.
  </action>
  <verify>
  ...
  <done>
    - Гороскоп сопровождается космическим изображением
    - Изображение отправляется отдельным сообщением
  </done>
</task>
...
1. Добавить импорты:
   ...
   from src.services.image_asset import get_image_asset_service
   ```
   (Bot уже импортирован если нужен)
...
2. Модифицировать `tarot_draw_three_cards(callback, session, state)`:
   ...
   - Bot доступен через callback.bot
...
3. Модифицировать `tarot_draw_celtic_cards(callback, session, state)`:
   - Аналогично — изображение перед media group

4. НЕ добавлять изображение для card_of_day — там уже есть изображение карты.

5. Получение bot в callback handlers:
   - В aiogram 3.x: `callback.bot` или добавить `bot: Bot` в signature
  </action>
  <verify>
  ...
  <done>
    - 3-card spread показывает космическое изображение
    - Celtic Cross показывает космическое изображение
...
3. Service usage:
   ```bash
   grep -c "get_image_asset_service" src/bot/handlers/*.py
   ```
   Ожидаемый результат: 3+ вхождения
</verification>

<success_criteria>
1. start.py отправляет изображение при /start
2. horoscope.py отправляет изображение при показе гороскопа
3. tarot.py отправляет изображение при 3-card и Celtic Cross
