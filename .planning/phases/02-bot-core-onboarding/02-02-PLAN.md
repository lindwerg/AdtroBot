...
must_haves:
  truths:
    ...
    - "Бот парсит дату в разных форматах (15.03.1990, 15 марта 1990)"
    - "Бот определяет знак зодиака по дате"
    - "Пользователь получает mock-гороскоп сразу после регистрации"
    - "Главное меню показывает 4 кнопки в сетке 2x2"
    - "Данные сохраняются в БД (birth_date, zodiac_sign)"
  artifacts:
...
<objective>
Complete onboarding flow: /start command, birthdate collection via FSM, zodiac determination, immediate value (mock horoscope), main menu.

Purpose: User can register, get first forecast, and see main menu with navigation.
Output: Working /start -> birthdate -> zodiac -> horoscope -> menu flow.
</objective>
...
<task type="auto">
    ...
    2. Create src/bot/handlers/start.py (main onboarding logic):
       ...
       - waiting_birthdate message handler:
         ...
         - If success:
           ...
           - Update birth_date and zodiac_sign
           - Commit session
           - Clear FSM state
  ...
  <done>
    ...
    Zodiac determined and saved to DB.
...
4. Mock horoscope shown after registration
...
- User.birth_date and User.zodiac_sign saved to DB
- Mock horoscope shown immediately after registration
- Main menu has 4 buttons: Гороскоп, Таро, Подписка, Профиль
- Menu buttons respond (horoscope for registered, teasers for others)
- Unknown messages handled gracefully
</success_criteria>
