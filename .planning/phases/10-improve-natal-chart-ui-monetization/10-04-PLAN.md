...
must_haves:
  truths:
    - "Бесплатные пользователи видят карту PNG с кнопкой под ней + краткое описание 300 слов"
  ...
  artifacts:
    ...
    - path: "src/bot/keyboards/natal.py"
      ...
      contains: "get_natal_with_buy_keyboard"
  key_links:
...
<task type="auto">
  <name>Task 1: Обновить NatalChartPrompt и добавить callback actions</name>
  <files>src/services/ai/prompts.py, src/bot/callbacks/natal.py</files>
  <action>
1. В src/services/ai/prompts.py обновить NatalChartPrompt для краткой бесплатной версии:
...
2. В src/bot/callbacks/natal.py добавить новые actions в enum NatalAction:
...
print('NatalChartPrompt SYSTEM length:', len(NatalChartPrompt.SYSTEM))
...
```
  ...
  <done>NatalChartPrompt обновлен для краткой версии, NatalAction расширен.</done>
</task>

<task type="auto">
  <name>Task 2: Создать keyboards для different user states</name>
  <files>src/bot/keyboards/natal.py</files>
  <action>
...
2. Добавить три новых keyboard функции:
...
kb3 = get_free_natal_keyboard()

print('Buy keyboard buttons:', len(kb1.inline_keyboard))
print('Open keyboard buttons:', len(kb2.inline_keyboard))
print('Free keyboard buttons:', len(kb3.inline_keyboard))
print('Buy button text contains price:', '199' in kb1.inline_keyboard[0][0].text)
"
```
  </verify>
  <done>Keyboards для всех трех user states созданы.</done>
</task>

<task type="auto">
  <name>Task 3: Обновить show_natal_chart для кнопки под фото</name>
  <files>src/bot/handlers/natal.py</files>
  <action>
ВАЖНО: Telegram не поддерживает клики по фото. Кнопка ДОЛЖНА быть под фото (reply_markup у photo сообщения).

1. Добавить импорты в начало файла handlers/natal.py:
```python
import asyncio
from src.bot.keyboards.natal import (
...
2. Найти функцию show_natal_chart() и обновить логику отправки фото.
   После генерации PNG и интерпретации, ПЕРЕД отправкой фото:
...
await callback.message.answer_photo(
    photo=BufferedInputFile(png_data, filename="natal_chart.png"),
    caption="Твоя натальная карта",
    reply_markup=keyboard,  # THIS IS KEY - button appears under photo
)
...
КРИТИЧНО: reply_markup в answer_photo() делает кнопку под фото. Это единственный способ в Telegram.
  </action>
  <verify>
```bash
cd /Users/kirill/Desktop/AdtroBot && python -c "
import inspect
from src.bot.handlers import natal

# Check imports
print('get_natal_with_buy_keyboard imported:', 'get_natal_with_buy_keyboard' in dir(natal))
print('get_telegraph_service imported:', 'get_telegraph_service' in dir(natal))

# Check show_natal_chart has reply_markup
source = inspect.getsource(natal.show_natal_chart) if hasattr(natal, 'show_natal_chart') else ''
print('reply_markup in show_natal_chart:', 'reply_markup' in source)
print('answer_photo in show_natal_chart:', 'answer_photo' in source)
"
```
  </verify>
  <done>show_natal_chart отправляет фото с кнопкой reply_markup под ним.</done>
</task>

<task type="auto">
  <name>Task 4: Добавить handler для BUY_DETAILED</name>
  <files>src/bot/handlers/natal.py</files>
  <action>
Добавить новый handler для покупки детального разбора:
...
ВАЖНО: Добавить импорт InlineKeyboardMarkup, InlineKeyboardButton если не импортирован:
```python
...
```bash
cd /Users/kirill/Desktop/AdtroBot && python -c "
from src.bot.handlers.natal import buy_detailed_natal
import inspect

sig = inspect.signature(buy_detailed_natal)
print('buy_detailed_natal params:', list(sig.parameters.keys()))

source = inspect.getsource(buy_detailed_natal)
print('Creates payment:', 'create_payment' in source)
print('Double-buy protection:', 'detailed_natal_purchased_at' in source)
print('Premium check:', 'is_premium' in source)
"
```
  </verify>
  <done>buy_detailed_natal handler создает платеж с защитой от двойной покупки.</done>
</task>

<task type="auto">
  <name>Task 5: Добавить handler для SHOW_DETAILED</name>
  <files>src/bot/handlers/natal.py</files>
  <action>
Добавить handler для показа купленного детального разбора:
...
ВАЖНО: Убедиться что _split_text и MAX_MESSAGE_LENGTH уже определены или добавить:
```python
...
```bash
cd /Users/kirill/Desktop/AdtroBot && python -c "
from src.bot.handlers.natal import show_detailed_natal
import inspect

sig = inspect.signature(show_detailed_natal)
print('show_detailed_natal params:', list(sig.parameters.keys()))

source = inspect.getsource(show_detailed_natal)
print('Uses Telegraph:', 'telegraph' in source.lower())
print('Uses generate_detailed_natal_interpretation:', 'generate_detailed_natal_interpretation' in source)
print('Saves to DetailedNatal:', 'DetailedNatal(' in source)
print('Has fallback chunks:', '_split_text' in source or 'chunks' in source)
"
```
  </verify>
  <done>show_detailed_natal генерирует, кэширует и показывает детальный разбор через Telegraph.</done>
</task>
...
4. Фото отправляется с reply_markup (кнопка под фото)
5. buy_detailed_natal handler создает платеж YooKassa
6. show_detailed_natal handler показывает/генерирует детальную интерпретацию
7. Double-buy protection работает (проверка detailed_natal_purchased_at)
8. Кэширование в DetailedNatal модели работает
</verification>
...
- [ ] Фото имеет reply_markup с кнопкой
