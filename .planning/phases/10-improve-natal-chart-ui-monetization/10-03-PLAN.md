...
<task type="auto">
  ...
  <action>
1. Добавить импорт DetailedNatalPrompt:
```python
from src.services.ai.prompts import DetailedNatalPrompt
```

2. Добавить импорт валидаторов:
```python
from src.services.ai.validators import validate_detailed_natal_section
```

3. Добавить кэш для детальной интерпретации (7 дней = 604800 секунд):
```python
...
4. Добавить метод generate_detailed_natal_interpretation в класс AIService:
...
ВАЖНО: Использовать существующий метод self._generate() для вызова API.
  </action>
  <verify>
```bash
cd /Users/kirill/Desktop/AdtroBot && python -c "
...
"
```
Должно показать: Method exists: True, Parameters: ['user_id', 'natal_data']
  </verify>
