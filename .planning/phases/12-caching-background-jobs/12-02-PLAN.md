...
must_haves:
  truths:
    ...
    - "Background job generates 12 horoscopes every 24 hours (00:00 Moscow time)"
  ...
  artifacts:
    ...
    - path: "src/services/scheduler.py"
      provides: "Background job for daily horoscope generation at 00:00 Moscow time"
...
<objective>
Implement HoroscopeCacheService with PostgreSQL persistence, background generation once daily at 00:00, and cache warming on startup

Purpose: Horoscopes load instantly from cache (<500ms), AI generation happens in background at 00:00 Moscow time (once per day for all 12 zodiac signs), cache is preloaded on restart
Output: Cache service with per-key locking + scheduled daily job at midnight + startup cache warming
</objective>
...
<task type="auto">
  <name>Task 2: Add background job for horoscope generation at 00:00 daily</name>
...
1. **New async function `generate_daily_horoscopes()`:**
   ...
   - Import HoroscopeCache from src.db.models.horoscope_cache
...
2. **Register job in get_scheduler() with 24-hour schedule (midnight only):**
...
**Key points:**
- `hour='0'` runs ONLY at 00:00 Moscow time (once per day, 24-hour schedule)
...
- Explicit DELETE cleanup BEFORE generation loop (not delegated to cache service)
...
2. `grep -E "hour='0'" src/services/scheduler.py | grep -v "hour='0,12'" && echo "24h schedule OK"`
  </verify>
  <done>
generate_daily_horoscopes job registered to run once daily at 00:00 Moscow time (24-hour schedule), includes explicit DELETE cleanup before generation, uses HoroscopeCacheService
  </done>
</task>
...
4. Scheduler has generate_daily_horoscopes job with CronTrigger(hour='0') - midnight only
5. Job explicitly deletes old horoscopes BEFORE generating new ones
...
- Background job scheduled at 00:00 Moscow time only (once per day, 24-hour cycle)
- Background job includes explicit DELETE FROM horoscope_cache WHERE date < today
- Background job uses HoroscopeCacheService for single-point-of-entry
- Cache warmed from PostgreSQL on application startup
</success_criteria>

<output>
After completion, create `.planning/phases/12-caching-background-jobs/12-02-PLAN.md`
</output>
