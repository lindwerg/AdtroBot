...
- View tracking
  ...
  <verify>
`python -c "from src.bot.utils.horoscope import get_horoscope_text; import asyncio; print(asyncio.get_event_loop().run_until_complete(get_horoscope_text('Aries', 'Овен'))[:50] if True else 'skip')"` runs without import errors
  </verify>
  <done>
get_horoscope_text uses HoroscopeCacheService for PostgreSQL-backed caching
  </done>
</task>
...
Use `today_start.date()` to get just the date part for comparison with HoroscopeView.view_date
  ...
  <verify>
`python -c "from src.admin.services.analytics import get_dashboard_metrics; print('imports OK')"` succeeds
  </verify>
  <done>
...
2. No more direct AI service call in horoscope.py (delegated to cache service)
3. Admin analytics imports HoroscopeView
4. horoscopes_today query sums view_count from horoscope_views WHERE view_date = today
5. Trend calculation compares today vs yesterday
</verification>
