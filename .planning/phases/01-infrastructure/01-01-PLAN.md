...
must_haves:
  truths:
    - "Зависимости установлены (poetry.lock существует)"
    - "Database schema определена (User model с telegram_id)"
    - "Миграция создана (файл в migrations/versions/)"
    - "Health endpoint доступен (GET /health возвращает 200)"
  artifacts:
    - path: "pyproject.toml"
