...
must_haves:
  truths:
    - "Проект запускается локально командой `uvicorn src.main:app`"
    - "GET /health возвращает HTTP 200 с телом {status: ok}"
    - "Database schema готова к применению (миграция создана, структура User корректна)"
    - "Конфигурация загружается из .env файла без ошибок"
  artifacts:
    - path: "pyproject.toml"
...
notes:
  - "INFRA-04 требует aiogram 3.x — добавляется в Phase 2 когда появится Telegram код"
  - "Phase 1 фокус: async SQLAlchemy + asyncpg (aiogram не используется без бота)"
---
...
Проверить что миграция содержит create_table("users") с правильными колонками.
  ...
  <verify>
Проверка файлов (без running PostgreSQL):
- `ls migrations/versions/*.py` — файл миграции существует
- `grep -l "create_table.*users" migrations/versions/*.py` — миграция содержит create_table для users
- `grep -l "telegram_id" migrations/versions/*.py` — миграция содержит telegram_id колонку
- `python -c "from src.main import app; print(app.routes)"` — FastAPI app импортируется

Для полной проверки с БД (выполняется в Plan 02 после Railway setup):
- `poetry run alembic upgrade head` применяет миграцию
- `curl http://localhost:8000/health` возвращает {"status":"ok"}
  </verify>
  <done>
Alembic настроен, первая миграция создана, FastAPI health check готов к запуску.
  </done>
</task>
...
3. `ls migrations/versions/*.py` — миграция создана
4. `grep create_table migrations/versions/*.py` — содержит create_table("users")
5. `poetry run python -c "from src.main import app"` — FastAPI app импортируется
6. `poetry run ruff check .` — без критических ошибок
</verification>
