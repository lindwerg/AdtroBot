...
must_haves:
  truths:
    ...
    - "Подписка автоматически продлевается через recurring payment"
  artifacts:
...
3. Update src/services/scheduler.py - add subscription expiry notification job:
           ...
           for subscription, user in to_renew:
               try:
                   ...
                   if payment.status == "succeeded":
                       ...
                       from src.services.payment.schemas import PLAN_DURATION_DAYS
                       subscription.current_period_start = subscription.current_period_end
                       subscription.current_period_end = subscription.current_period_end + timedelta(days=PLAN_DURATION_DAYS[plan])
   ...
   # In get_scheduler(), add the jobs:
   # After existing jobs, add:

   # Auto-renewal job (runs before expiry notification)
   scheduler.add_job(
  ...
  <done>
    ...
    - Scheduler job auto_renew_subscriptions runs at 09:00 daily
    - Scheduler job check_expiring_subscriptions runs at 10:00 daily
    - Failed renewals mark subscription as past_due and notify user
  </done>
</task>
...
3. `poetry run python -c "from src.services.scheduler import get_scheduler; s = get_scheduler(); jobs = [j.id for j in s.get_jobs()]; assert 'check_expiring_subscriptions' in jobs; assert 'auto_renew_subscriptions' in jobs; print('Scheduler OK')"`
...
- Atomic limit check prevents race conditions
- Subscriptions auto-renew 1 day before expiry using saved payment method
- Failed renewals mark subscription as past_due and notify user
