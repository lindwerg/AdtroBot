...
must_haves:
  truths:
    - "Скрипт готов к генерации 17 изображений"
    - "Промпты обеспечивают единый визуальный стиль (космос, золото, мистика)"
    - "Тестовый запуск подтверждает работоспособность SDK и API"
  artifacts:
    - path: "pyproject.toml"
...
**Промпты для генерации (17 изображений):**
...
- Добавить --dry-run флаг для проверки без API вызовов
  </action>
  <verify>
...
- `python scripts/generate_images.py --dry-run` выводит список 17 изображений без API вызовов
  </verify>
  <done>Скрипт готов к запуску, содержит все промпты, dry-run подтверждает работоспособность</done>
</task>
...
3. `python scripts/generate_images.py --dry-run` выводит список всех 17 изображений и завершается без ошибок
...
- Dry-run успешно проходит (SDK + скрипт работают)
</success_criteria>
