...
3. Create natal_svg.py with generate_natal_png():
  ...
  <done>
    ...
    Timezone properly converted to UTC for calculations.
  </done>
</task>
...
5. Register natal router:
   - Export router in `src/bot/handlers/__init__.py`
   - Add router to main.py dispatcher (after menu router, before common)

6. Add menu handler for "Натальная карта" text in handlers/menu.py:
   - Route to natal handler
  </action>
  <verify>
