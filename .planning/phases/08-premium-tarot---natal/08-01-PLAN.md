...
must_haves:
  truths:
    ...
    - "Premium users have 20 spreads/day limit, free users have 1 spread/day"
  artifacts:
...
- Updated spread limits: premium 20/day, free 1/day
</objective>
...
<task type="auto">
  <name>Task 2: Celtic Cross handlers + History UI + Spread limits</name>
  <files>
    src/bot/callbacks/tarot.py
    src/bot/keyboards/tarot.py
    src/bot/handlers/tarot.py
...
7. Update spread limit checks (TAROT-08 requirement):
   - DAILY_SPREAD_LIMIT_FREE = 1 (unchanged)
   - DAILY_SPREAD_LIMIT_PREMIUM = 20
   - In limit check function: use is_premium to determine which limit applies
   - Update remaining limit display to show correct max based on subscription status

8. Celtic Cross card positions (use these names):
   ["Настоящее", "Препятствие", "Прошлое", "Будущее", "Сознательное", "Подсознательное", "Я", "Окружение", "Надежды/Страхи", "Исход"]

9. Premium teaser for Celtic Cross (free users):
  ...
  <done>
    ...
    Premium users have 20/day limit, free users have 1/day limit.
  </done>
</task>
...
- Premium users have 20 spreads/day, free users have 1 spread/day
</success_criteria>
