...
must_haves:
  truths:
    - "Admin can create A/B experiments with variant assignments (A/B split)"
  ...
  artifacts:
    - path: "src/admin/models.py"
      provides: "ABExperiment and ABAssignment models"
...
<task type="auto">
  ...
  <verify>
    ...
    - psql -c "SELECT COUNT(*) FROM ab_experiments" (succeeds, returns 0)
    - psql -c "SELECT COUNT(*) FROM ab_assignments" (succeeds, returns 0)
    - psql -c "SELECT utm_source FROM users LIMIT 1" (field exists, no error)
  </verify>
    ...
    3. Create admin-frontend/src/pages/ABTests.tsx:
       ...
       export default function ABTestsPage() {
         ...
         const columns = [
           ...
           { dataIndex: 'metric', title: 'Метрика' },
           {
           ...
           <Row gutter={[16, 16]}>
             ...
             >
               ...
               >
                 ...
                 <Form.Item name="metric" label="Метрика" rules={[{ required: true }]}>
                   <Select options={[
                 ...
                 <Form.Item name="variant_b_percent" label="% пользователей в варианте B" initialValue={50}>
             ...
             >
               {results && (
                 <Row gutter={[16, 16]}>
                   <Col span={12}>
                     <Card size="small" title={`Вариант A ${results.winner === 'A' ? '🏆' : ''}`}>
                       <Statistic title="Пользователей" value={results.variant_a.users} />
                       <Statistic title="Конверсий" value={results.variant_a.conversions} />
                       <Statistic
                         title="Конверсия"
                   ...
                   <Col span={12}>
                     <Card size="small" title={`Вариант B ${results.winner === 'B' ? '🏆' : ''}`}>
                       <Statistic title="Пользователей" value={results.variant_b.users} />
                       <Statistic title="Конверсий" value={results.variant_b.conversions} />
                       <Statistic
                         title="Конверсия"
                   ...
                   {results.winner && (
                     <Col span={24}>
                       <Typography.Text type="success">
                         <TrophyOutlined /> Победитель: Вариант {results.winner}
                       </Typography.Text>
                     </Col>
                   )}
                 </Row>
